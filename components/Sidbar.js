...
function Sidbar() {
    ...
    const createChat = () => {
        const input = prompt('Please Enter and email address for the user you wish to chat with');
        if (!input) return null;

        if (EmailValidator.validate(input) && !chatAlreadyExits(input) && input !== user.email) {
            // we need to add the chat into the DB 'chats' collection if it doesn't already exit and valid
